...
#						$debug && print STDERR "DEBUG: p2p: building update for ".
#								"protocol: $ipprotocol ".
#								"type: $rrdtype ".
...
#								"\n";
					...
					$debug && print STDERR "DEBUG: individual: building update for ".
							...
							"type: $rrdtype ".
							"protocol: $ipprotocol ".
							"cust: $srcvli / '$matrix->{names}->{$srcvli}' ".
							"\n";
					build_update_rrd ($rrdfile, $rrdtype, $ipprotocol, $in, $out, $interval, $rrdcached);
				}
				...
				$debug && print STDERR "DEBUG: aggregate: building update for vlan: $vlan type: $rrdtype protocol: $ipprotocol\n";
