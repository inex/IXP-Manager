        ...
        <div class="col-sm-12">
            ...
            <nav id="filter-row" class="navbar navbar-expand-lg navbar-light bg-light mb-4 shadow-sm">
                ...
                <div class="collapse navbar-collapse" id="navbarNavDropdown">
                    <form class="navbar-form navbar-left form-inline d-block d-lg-flex"  action="<?= route( "statistics@core-bundle", [ "cbid" => $t->cb->getId() ] ) ?>" method="GET">
                ...
                <div class="pull-right tw-text-gray-600">
                    ...
                    <?= $t->ee( $this->cb->getSwitchSideX( $t->graph->side() === 'a' )->getName() )  ?> -
                    <?= $t->ee( $this->cb->getSwitchSideX( $t->graph->side() !== 'a' )->getName() )  ?>,

                    <?php if( count( $this->cb->getCoreLinks() ) > 1 ): ?>
                        <?= count( $this->cb->getCoreLinks() ) ?> x <?= $t->scaleBits( $this->cb->getSpeedPi() * 1000000, 0 ) ?>
                        = <?= $t->scaleBits( count( $this->cb->getCoreLinks() ) * $this->cb->getSpeedPi() * 1000000, 0 )  ?>
                    <?php else: ?>
