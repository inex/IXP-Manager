...
int set allas;
{
    ...
    <?= $t->insert( 'api/v4/router/collector/bird3/import-pre-extra', [ 'int' => $int ] ) ?>
...
<?php
// If config has default prefix limit set and peer maxprefixes is
// higher than this, use this instead of default. Otherwise use default
// set by tb_rcpeer neighbor template.
if(($cfg_limit = (int) config("custom.router.{$t->handle}.prefix_limit")) && ($int['maxprefixes'] > $cfg_limit)):
        $peer_rc_max = $int['maxprefixes'];
else:
        $peer_rc_max = 0;
endif;
?>

protocol bgp pb_as<?= $int['autsys'] ?>_vli<?= $int['vliid'] ?>_ipv<?= $int['protocol'] ?? 4 ?> from tb_rcpeer {
    description "AS<?= $int['autsys'] ?> - <?= $int['cname'] ?>";
    neighbor <?= $int['address'] ?> as <?= $int['autsys'] ?>;

    <?= $t->insertif('api/v4/router/collector/bird3/peer_as_' . $int['autsys'] ) ?>
    ...
    <?= $t->ipproto ?> {
        ...
        import limit <?= (int) $int['maxprefixes']?> action warn;
        <?php if( $peer_rc_max > 0 ): ?>receive limit <?= $peer_rc_max ?>;<?php endif; ?>
